    ...
    api.onmessage = function(msg, data)  {
      if ("event" in data) {
        ...
        if (data.event == "UpdateEntity") {
          ...
          var div = document.getElementById("drone"+ data.details.id);
          if(div != undefined){
            updateBattery(div, data.details.bat);
          }
...
function addEntity(data) {
  ...
  if(data.details.type == "drone"){
    var droneDiv = document.createElement("div");
    droneDiv.id = "drone"+ data.id;
    droneDiv.classList.add("droneInfo");


    var temp = document.createElement("div");
    temp.classList.add("name");
    temp.innerHTML += data.details.name + ":";
    droneDiv.appendChild(temp);

    temp = document.createElement("div");
    temp.classList.add("battery");
    droneDiv.appendChild(temp);

    for(var i = 0; i<batteryLevels; i++){
      var temp2 = document.createElement("div");
      temp2.classList.add("batteryLevel");
      temp2.value = i;
      temp.appendChild(temp2);
    }

    temp = document.createElement("div");
    temp.classList.add("batteryNub");
    droneDiv.appendChild(temp);

    document.getElementById("notification-bar").appendChild(droneDiv);
  }
...
function updateBattery(div, batPercent){
  var levels = div.getElementsByClassName("battery")[0].children;


  var lastFull = batteryLevels * batPercent;

  var filledColor = "#1DE420";
  if(lastFull < batteryLevels*0.5){
    filledColor = "#ff9900";
  }
  if(lastFull < batteryLevels*0.25){
    filledColor = "#e41d1d";
  }

  for(var i = 0; i < batteryLevels; i++){
    if(i < lastFull){
      levels[i].style.backgroundColor = filledColor;
    }
    else{
      levels[i].style.backgroundColor = "#000";
    }
  }
